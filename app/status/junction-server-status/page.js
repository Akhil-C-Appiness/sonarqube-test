...
import { getAllJunctionStatus, getSiteStatus } from "@/lib/api"
        ...
        <div className="w-full px-8">
            <table className="w-full">
                <tr>
                    ...
                    <th className="h-[56px] bg-[#EEF8FF] p-4 text-left text-sm font-semibold leading-6 text-[#0F0F10]">Local Storage</th>
                    <th className="h-[56px] bg-[#EEF8FF] p-4 text-left text-sm font-semibold leading-6 text-[#0F0F10]">Camera Alert</th>
                    <th className="h-[56px] bg-[#EEF8FF] p-4 text-left text-sm font-semibold leading-6 text-[#0F0F10]">Action</th>
                </tr>
                {junctionArray?.map((item,index) => (
                    <tr className={index%2 == 0 ? trodd : treven}>
                        ...
                        <td className={tdstyle}><Link href={`junction-server-status/${item.componnetId}`}><Button variant="default" className="h-6 py-0" >More</Button></Link></td>
                    </tr>
                ))}
            </table>
        </div>
