...
export default function ProfileForm({setOpenAdminProfile, setOpenAdminPassword}) { 
  ...
  const onSubmit = async(values) => {
    ...
    if(response.status === 200){
        ...
        let responseObj = response.data.result[0];
        userInfo.id = responseObj.id
        userInfo.password = responseObj.password
        userInfo.mailId = responseObj.mailId
        userInfo.mobile = responseObj.mobile
        userInfo.sequrityQuestion1 =responseObj.sequrityQuestion1
        userInfo.sequrityQuestion2 = responseObj.sequrityQuestion2
        userInfo.sequrityAnswer1 = responseObj.sequrityAnswer1
        userInfo.sequrityAnswer2 = responseObj.sequrityAnswer2
        localStorage.setItem("user-info", JSON.stringify(userInfo));

        console.log("response.result------",response.data.result[0])
    }
    else{
        toast({
            variant: "destructive",
