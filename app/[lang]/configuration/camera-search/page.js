...
const CameraSearch = () => {
  ...
  const handleSearch = async () => {
    if (!username || !password) {
      toast({
        variant: "destructive",
        description: dictionary?.pleaseEnterUsernameAndPassword,
        duration: 3000,
      })
      return
    }
    if (!storeModel || !selectedMSId?.mediaServerId) {
      toast({
        variant: "destructive",
        description: dictionary?.pleaseEnterJunctionAndMedia,
        duration: 3000,
      })
      return
    }
      ...
      while (shouldContinueRef.current) {
        ...
        if (response.data.code === 103) {
          toast({
            variant: "success",
            description:
              dictionary?.searchSuccessful +
              response.data.result[0].length +
              dictionary?.camerasFound,
            duration: 3000,
          })
  ...
  const stopSearchHandler = async () => {
    ...
    try {
      ...
      if (response.data.message) {
        toast({
          variant: "success",
          description: dictionary?.stopSearchSuccessfullyApplied,
          duration: 3000,
        })
      } else {
        toast({
          variant: "destructive",
          description: dictionary?.stopSearchFailed,
          duration: 3000,
        })
      }
    } catch (error) {
      console.error(dictionary?.errorWhileStoppingSearch, error)
    }
  }
  ...
  const fetchCamSnap = async (selectedCameraIndex) => {
    try {
      ...
      toast({
        variant: "destructive",
        description: dictionary?.unauthorizedUnableToRetrieveSnap,
        duration: 3000,
      })
    } finally {
      setLoading(false)
  ...
  const fetchSearchDetails = async (selectedCameraIndex) => {
    try {
      ...
      if (searchDetails.data.message) {
        toast({
          variant: "success",
          description: dictionary?.successfullySaved,
          duration: 3000,
        })
      }
      ...
      toast({
        variant: "destructive",
        description: dictionary?.errorOccurredWhileSaveChanges,
        duration: 3000,
      })
    }
  }
  ...
  function getStreamType(recordingId) {
    switch (recordingId) {
      case 0:
        return dictionary?.majorStream
      case 1:
        return dictionary?.minorStream
      case 2:
        return `MJPG ${stream}`
      case 3:
        return "Jpeg Snap"
  ...
  const fixedOptions = ["ANPR", "Evidence", dictionary?.others]
  useEffect(() => {
    const cameraTypeMap = {
      0: "ANPR",
      1: "Evidence",
  ...
  const addCamHandler = async () => {
    if (!storedSnapUrl || storedSnapUrl === null) {
      toast({
        variant: "destructive",
        description: dictionary?.pleaseSaveChangesBeforeAddingACamera,
        duration: 3000,
      })
      return
    } else {
      ...
      if (camSelected.data.message) {
        toast({
          variant: "success",
          description: dictionary?.successfullyAdded,
          duration: 3000,
        })
        ...
        toast({
          variant: "destructive",
          description: dictionary?.dBMSUnknownErrorPleaseReportTheProblem,
          duration: 3000,
        })
  ...
  return (
    <div className="w-full p-4" dir={dictionary?.direction || "ltr"}>
      <div className="w-full">
        ...
        >
          <AccordionItem value="item-1">
            <AccordionTrigger>{dictionary?.userCredentials}</AccordionTrigger>
            <AccordionContent>
              <div className="flex flex-row">
                <div className="p-4">
                  <Label for={dictionary?.userName}> {dictionary?.userName} </Label>
                  <Input
                ...
                <div className="p-4">
                  <Label for={dictionary?.password}> {dictionary?.password} </Label>
                  <Input
      ...
      <div className="py-4">
        <div className="flex">
          <div>{dictionary?.search} {dictionary?.by} {dictionary?.camera}</div>
        </div>
        <div className="flex flex-row gap-4 pt-4">
          <div>
            <Select onValueChange={(val) => setStoreId(val)}>
              ...
              <SelectContent className="max-h-40 overflow-y-scroll">
                <SelectGroup>
                  <SelectLabel>{dictionary?.junction}</SelectLabel>
          ...
          <div>
            <Select onValueChange={(value) => setStoreModel(value)}>
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder={dictionary?.model} />
              </SelectTrigger>
              <SelectContent>
                <ScrollArea className="h-72 w-48 rounded-md border">
                  <SelectGroup>
                    <SelectLabel>{dictionary?.model}</SelectLabel>
          ...
          <div>
            ...
            >
              {isLoading ? `${dictionary?.stop} ${dictionary?.search}` : dictionary?.search}
            </Button>
          </div>
      ...
      <div className="flex gap-4">
        <div className="w-72 h-[360px] bg-blue-100 py-4">
          ...
          <div class="h-72 overflow-y-scroll scrollbar-thin scrollbar-thumb-blue-500 scrollbar-track-gray-200">
            <div className="px-4">{lenCam} {dictionary?.camerasFound}</div>
          ...
          <div className="w-full grow border bg-slate-50 p-2">
            <div className="flex">
              <div className="text-semibold">
                {dictionary?.camera} {selectedCameraIndex + 1}
              </div>
            </div>
            <div className="pt-4">
              {loading ||
                ...
                <AspectRatio ratio={16 / 9} className="bg-gray-900">
                  <div className="flex h-full w-full flex-col items-center justify-center">
                    <Image
                      ...
                      height={80}
                    />
                    <p className="bg-[#fff] px-2">{dictionary?.imageIsNotAvailable}</p>
                  </div>
                </AspectRatio>
              ) : (
                // <Image
            ...
            <div className="pt-4">
              <div className="flex gap-4 text-slate-400">
                ...
                <div>{dictionary?.model} - {data[selectedCameraIndex].channel.model}</div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="mt-4 w-full ">
                  <div>{dictionary?.recordingStream}</div>
                  <div>
                    <Select>
                      ...
                      <SelectContent>
                        <SelectGroup>
                          <SelectLabel>{dictionary?.recordingStream}</SelectLabel>
                          <SelectItem value={dictionary?.majorStream}>
                           {dictionary?.majorStream}
                          </SelectItem>
                          <SelectItem value={dictionary?.minorStream}>
                            {dictionary?.minorStream}
                          </SelectItem>
                          <SelectItem value="MJPGStream">
                            MJPG {dictionary?.stream}
                          </SelectItem>
                ...
                <div className="mt-4 w-full ">
                  <div>{dictionary?.camera} {dictionary?.type}</div>
                  <div>
                    <Select
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectGroup>
                          <SelectLabel>{dictionary?.camera} {dictionary?.type}</SelectLabel>
                ...
                <div className="mt-4 w-full ">
                  <div>{dictionary?.laneNumber}</div>
                  <div>
                    <Select>
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder={dictionary?.laneNumber} />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectGroup>
                          <SelectLabel>{dictionary?.laneNumber}</SelectLabel>
              ...
              <div className="mt-4 w-full text-right">
                <Button onClick={handleSaveChanges}>{dictionary?.saveChanges}</Button>
              </div>
            </div>
          </div>
        )}
